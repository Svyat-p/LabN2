...
Для решения задачи также потребуются вспомогательные переменные для хранения сумм элементов диагоналей и разницы между ними(`sum`, `sum2` и `diff`) типа `int`, элемента на пересечении диагоналей(`middle`) типа `int`, сумм элементов строк и столбцов(`scom`) типа `int`, элементов диагоналей(`te`) типа `int`, индексов элементов массива(`i1` и `j1`) типа `int`, суммы строк и столбцов(`sumstr` и `sumcol`) типа `int`, индексов строк и столбцов(`indstr` и `indcol`) типа `int`, суммы первой строки и первого столбца(`smin`) типа `int`, среднего арифметического диагоналей(`srar`) типа `double`.
...
4. Тест на массив с четным размером:
    ...
    - **Output**:
        ...
        1 2
        6
        Индекс строки, при вычеркивании которой сумма останется максимальной: 1
        Индекс столбца, при вычеркивании которого сумма останется максимальной: 0
